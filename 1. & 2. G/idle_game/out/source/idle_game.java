...
public class idle_game extends PApplet {
...
float multiCost = 1.3f, multiMultiplier = 1;
float multiAmountCost = 1.75f, multiAmount = 1.2f;
float rebirthMulti = 1, rebirthCost = 100000;
int rebirth;
float clickerCost = 2.1f; 
int amountClicker;
...
public void buttonProgress() {
      fill(255);
      if (pow(multiCost, 2.5f) > value) {
        image(gameplayButtons11,15, height/3+150);
        text(nf(value/pow(multiCost, 2.5f)*100, 0, 1) + "%", 115, height/3+200);
        rect(25, height/3+210, constrain(value/pow(multiCost, 2.5f)*180, 1, 180), 5 , 20);
      } else{
        image(gameplayButtons1,15, height/3+150);
        text(nf(value/pow(multiCost, 2.5f)*100, 0, 1) + "%", 115, height/3+200);
        rect(25, height/3+210, constrain(value/pow(multiCost, 2.5f)*180, 0, 180), 5 , 20);
      }
...
public void controlEvent(CallbackEvent event) {
  ...
  if (event.getAction() == ControlP5.ACTION_CLICK) {
    ...
    case "/Multiplier":
      if (value >= pow(multiCost, 2.5f)) {
        value -= pow(multiCost, 2.5f);
        multiMultiplier += multiAmount;
        multiCost *= 1.25f;
        consolePrint();
      } else {
        println("not enough Credits - " + pow(multiCost, 2.5f) + " needed");
      }
      break;
    ...
    case "/Rebirth":
      if (value >= rebirthCost) {
        ...
        rebirthCost *= pow(2, 1.65f) ;
        rebirthMulti *= 1.8f;
        rebirth ++;
        value = 0; 
        multiCost = 1.3f; 
        multiMultiplier = 1; 
        multiAmountCost = 1.75f; 
        multiAmount = 1.2f;
        amountClicker = 0;
        clickerCost = 2.1f;
        consolePrint();
  ...
  public void settings() {  size(450, 720); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "idle_game" };
    if (passedArgs != null) {
